...
def opt_and_lambdify(fun, u_sym, v_sym, par_sym):
    fun = optimize(fun, optims_c99)
    ufun = lambdify((par_sym, u_sym, v_sym),
                    fun,
                    'numpy')
    return fun, ufun
...
def copula_derivs_one_par(cdf_sym, u_sym, v_sym, par_sym):
    ufuns = dict()
...
def sym_copula_derivs_one_par(cdf_sym, u_sym, v_sym, par_sym):
    sym_funs = dict()
    ...
    d_ll_d_par_sym = diff(ll_sym, par_sym)
    sym_funs['d_ll_d_par'] = opt_and_print(d_ll_d_par_sym)

    d_cdf_d_par_sym = diff(cdf_sym, par_sym)
    sym_funs['d_cdf_d_par'] = opt_and_print(d_cdf_d_par_sym)

    d_hfun_d_par_sym = diff(hfun_sym, par_sym)
    sym_funs['d_hfun_d_par'] = opt_and_print(d_hfun_d_par_sym)

    d_vfun_d_par_sym = diff(vfun_sym, par_sym)
    sym_funs['d_vfun_d_par'] = opt_and_print(d_vfun_d_par_sym)
